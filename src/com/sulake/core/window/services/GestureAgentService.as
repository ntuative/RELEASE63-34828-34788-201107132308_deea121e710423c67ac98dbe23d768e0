package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_363,this.clientWindowDestroyed);
            this.var_316 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1424 *= 0.75;
         this.var_1423 *= 0.75;
         if(Math.abs(this.var_1424) <= 1 && Math.abs(this.var_1423) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_316 != null)
         {
            this.var_316(this.var_1424,this.var_1423);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_363,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
