package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_981:int = 3;
      
      private var var_447:AssetLoaderStruct;
      
      private var var_979:Timer;
      ...
      private const const_2349:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_447 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_979 = new Timer(this.const_2349,1);
         this.var_979.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_447)
         {
            this.var_447.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_447.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_447)
         {
            this.var_447.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_447.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_981 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1201);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_981);
            if(this.var_1201.indexOf("?") > 0)
            {
               _loc2_ = this.var_1201 + "&retry=" + this.var_981;
            }
            else
            {
               _loc2_ = this.var_1201 + "?retry=" + this.var_981;
            ...
            this.var_447 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         for each(_loc2_ in param1.lib)
         {
            ...
            for each(_loc4_ in _loc2_.part)
            {
               ...
               _loc6_ = this.var_1202[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1202[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1794)
         {
            this.var_1200.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_590[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_590[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_788.length)
         {
            _loc5_ = this.var_788[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_788.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_979.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_789.length > 0 && this.var_788.length < this.const_2006)
         {
            _loc1_ = this.var_789[0];
            _loc1_.startDownloading();
            this.var_788.push(this.var_789.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_789.indexOf(param1) == -1 && this.var_788.indexOf(param1) == -1)
         {
