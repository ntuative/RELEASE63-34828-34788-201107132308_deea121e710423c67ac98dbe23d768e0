package com.sulake.habbo.toolbar.extensions
{
   ...
   public class ClubDiscountPromoExtension
   {
      ...
      private static const const_1665:int = 13;
      
      private static const const_1133:int = 14;
      
      private static const const_1663:uint = 16777215;
      
      private static const const_1664:uint = 12247545;
         ...
         if(_loc4_)
         {
            ...
            if(_loc3_)
            {
               ...
               if(this.var_89)
               {
                  ...
                  if(_loc5_)
                  {
                     this.var_886 = _loc5_.content as BitmapData;
                     if(this.var_886)
                     {
                        this.var_89.bitmap = this.var_886.clone();
                     }
                  }
                  this.var_89.visible = false;
               }
               this.var_1342 = _loc3_.findChildByName("text_region") as IRegionWindow;
               if(this.var_1342)
               {
                  this.var_1342.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onTextRegionClicked);
                  this.var_1342.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.onTextRegionMouseOver);
                  this.var_1342.addEventListener(WindowMouseEvent.const_25,this.onTextRegionMouseOut);
               }
               this.assignState();
            }
         }
      ...
      public function dispose() : void
      {
         if(!this._disposed)
         {
            if(this.var_639)
            {
               this.var_639.detachExtension(const_712);
               this.var_639 = null;
            }
            this.animate(false);
            this.clearAnimation();
      ...
      private function onClubChanged(param1:HabboInventoryHabboClubEvent) : void
      {
         if(this.var_12.clubIsExpiring)
         {
            if(!this._view)
            {
               ...
               this.assignState();
               this.var_639.attachExtension(const_712,this._view,ExtensionFixedSlotsEnum.const_1917);
            }
         }
         else
         {
      ...
      private function animate(param1:Boolean) : void
      {
         if(param1)
         {
            ...
            this.var_712 = new Timer(15000);
            this.var_712.addEventListener(TimerEvent.TIMER,this.onTriggerTimer);
            this.var_712.start();
         }
         else
         {
      ...
      private function resetAnimationVariables() : void
      {
         ...
         this.var_2095 = this._view.width - 7 - this.var_89.bitmap.width;
         this.var_1644 = 0;
      }
      
      private function startAnimationTimer() : void
      {
         this.var_694 = new Timer(25,26);
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         this.var_89.x = 3 + this.var_1644 / 20 * this.var_2095;
         if(this.var_89.x > this.var_2095)
         {
            _loc2_ = this._view.width - 4 - this.var_89.x;
            _loc3_ = new BitmapData(_loc2_,this.var_886.height);
            _loc3_.copyPixels(this.var_886,new Rectangle(0,0,_loc2_,this.var_886.height),new Point(0,0));
